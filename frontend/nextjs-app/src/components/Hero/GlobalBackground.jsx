...
function NightSkyLayer() {
  const random = createSeededRandom(20260324);
  const stars = Array.from({ length: STAR_COUNT }, (_, i) => {
    const left = 2 + random() * 96;
    const top = 2 + Math.pow(random(), 1.12) * 96;
    const size = 0.8 + random() * 2.6;
    const opacity = 0.2 + random() * 0.72;
    const twinkle = random() > 0.72;
    return { id: i, left, top, size, opacity, twinkle };
  });
