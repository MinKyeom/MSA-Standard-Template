...
public class JwtValidationGlobalFilter implements GlobalFilter, Ordered {
    ...
    private static final List<Pattern> PUBLIC_PATTERNS = List.of(
        Pattern.compile("^/auth/(login|signup|send-code|verify-code|refresh|extend|logout).*"),
        Pattern.compile("^/auth/find-username/.*"),
        Pattern.compile("^/auth/reset-password/.*"),
        ...
        Pattern.compile("^/api/posts/visits/.*"),  // 조회수 기록·통계 (비로그인 허용)
