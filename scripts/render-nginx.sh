...
TEMPLATE="${ROOT}/nginx/msa-project.conf.template"
...
# nginx 의 $host 등은 치환하지 않도록 변수 이름만 나열 (GNU/BSD 공통)
VARS='$NGINX_HTTP_PORT $NGINX_HTTPS_PORT $PRIMARY_SERVER_NAMES $SSL_CERT_PATH $SSL_KEY_PATH $UPSTREAM_GATEWAY $UPSTREAM_FRONTEND $METRICS_ALLOW_IP $DEV_DOMAIN $DEV_SSL_CERT $DEV_SSL_KEY $DEV_UPSTREAM_FRONTEND $DEV_UPSTREAM_GATEWAY'
