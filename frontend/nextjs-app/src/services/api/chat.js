...
import { chatHttpUrl } from "../../config/apiBase";
...
export const sendChatMessage = async (session_id, message) => {
  try {
    const url = chatHttpUrl();
    if (!url) {
      return "챗봇 URL이 설정되지 않았습니다. NEXT_PUBLIC_CHATBOT_API_URL 또는 Docker의 GATEWAY_INTERNAL_URL을 확인하세요.";
    }
