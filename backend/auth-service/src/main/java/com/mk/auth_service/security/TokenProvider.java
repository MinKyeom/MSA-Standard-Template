...
public class TokenProvider {
    ...
    /**
     * 만료(access expired)된 JWT라도 subject(=userId)만 추출해서 로그아웃 시 Redis 무효화를 보강합니다.
     * - 서명 위변조 등 JwtException 계열이면 null
     * - ExpiredJwtException이면 subject를 반환
     */
    public String getUserIdFromTokenIgnoringExpiration(String token) {
        try {
            Claims claims = Jwts.parserBuilder()
                    .setSigningKey(getSigningKey())
                    .build()
