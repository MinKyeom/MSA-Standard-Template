...
import com.mk.auth_service.security.AuthCookieSupport;
...
public class AuthController {
    ...
    @PostMapping("/login")
    public ResponseEntity<?> login(@RequestBody @Valid SigninRequest request, HttpServletRequest httpRequest, HttpServletResponse response) {
        ...
        setCookie(httpRequest, response, COOKIE_AUTH_TOKEN, accessToken, accessExpiryMinutes * 60);
        setCookie(httpRequest, response, COOKIE_REFRESH_TOKEN, refreshToken, refreshExpiryDays * 24 * 60 * 60);
    ...
    public ResponseEntity<?> logout(HttpServletRequest request, HttpServletResponse response) {
        // Access token은 만료됐을 수 있으므로(subject는 만료된 토큰에서도 추출 가능),
        // 우선 refresh token 기준 무효화 → 실패/누락 시 access 토큰 subject 기반 무효화로 보강합니다.
        ...
        if (refreshToken != null) {
            try {
                ...
                // refresh token이 유효하지 않아도 아래에서 쿠키는 무조건 제거합니다.
            }
        }

        if (!invalidated && accessToken != null) {
            String userId = tokenProvider.getUserIdFromTokenIgnoringExpiration(accessToken);
            if (userId != null) {
                authService.invalidateRefreshByUserId(userId);
            }
        }
        clearCookie(request, response, COOKIE_AUTH_TOKEN);
        clearCookie(request, response, COOKIE_REFRESH_TOKEN);
        return ResponseEntity.ok().build();
    }
    ...
    public ResponseEntity<?> refresh(HttpServletRequest request, HttpServletResponse response) {
        ...
        setCookie(request, response, COOKIE_AUTH_TOKEN, newAccess, accessExpiryMinutes * 60);
    ...
    private void setCookie(HttpServletRequest request, HttpServletResponse response, String name, String value, int maxAgeSeconds) {
        AuthCookieSupport.addAuthCookie(request, response, name, value, maxAgeSeconds, cookieDomain);
    }
    ...
    private void clearCookie(HttpServletRequest request, HttpServletResponse response, String name) {
        AuthCookieSupport.clearAuthCookie(request, response, name, cookieDomain);
    }
}
